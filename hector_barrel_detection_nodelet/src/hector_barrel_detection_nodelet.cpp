...
void BarrelDetection::onInit()
{
  ...
  pcl_sub = nh_.subscribe( "/arm_rgbd_cam/depth/points", 1, &BarrelDetection::PclCallback, this );
  image_sub = it_.subscribeCamera( "/arm_rgbd_cam/rgb/image_raw", 10,
                                   &BarrelDetection::imageCallback, this );
  ...
  barrel_marker_publisher_ =
      pnh_.advertise<visualization_msgs::MarkerArray>( "marker_filtered_barrel", 0 );
  imagePercept_pub_ =
      pnh_.advertise<hector_worldmodel_msgs::ImagePercept>( "/worldmodel/image_percept", 0 );
  posePercept_pub_ =
      pnh_.advertise<hector_worldmodel_msgs::PosePercept>( "/worldmodel/pose_percept", 0 );
  ...
  worldmodel_srv_client_ = nh_.serviceClient<hector_nav_msgs::GetDistanceToObstacle>(
      "/move_group/get_distance_to_obstacle" );
...
void BarrelDetection::imageCallback( const sensor_msgs::ImageConstPtr &img,
                                     const sensor_msgs::CameraInfoConstPtr &info )
{
  ...
  img_filtered = img_filtered( cv::Rect( size.width * cutPercentage, size.height * cutPercentage,
                                         size.width * ( 1 - 2 * cutPercentage ),
                                         size.height * ( 1 - 2 * cutPercentage ) ) );

  //    cv::imshow("image",img_filtered);

  cv::Mat blueOnly;
  cv::inRange( img_filtered, cv::Scalar( h_min, s_min, v_min ), cv::Scalar( h_max, s_max, v_max ),
               blueOnly );
  ...
  for ( unsigned int i = 0; i < keypoints.size(); i++ ) {
    ...
    cv::Point2d rectified = cameraModel->rectifyPoint(
        cv::Point2d( ip.x + size.width * cutPercentage, ip.y + size.height * cutPercentage ) );
    ...
    try {
      ...
      listener_.waitForTransform( "/map", img->header.frame_id, time, ros::Duration( 3.0 ) );
...
void BarrelDetection::findCylinder( const sensor_msgs::PointCloud2::ConstPtr &pc_msg, float xKey,
                                    float yKey, const geometry_msgs::PointStamped cut_around_keypoint )
{
  ...
  try {
    ...
    listener_.waitForTransform( "/map", pc_msg->header.frame_id, time, ros::Duration( 3.0 ) );
    listener_.lookupTransform( "/map", pc_msg->header.frame_id, time, transform_cloud_to_map );
  } catch ( tf::TransformException ex ) {
    ROS_ERROR( "Lookup Transform failed: %s", ex.what() );
    return;
  }
  ...
  if ( cloud->points.size() > 0 ) {
    ...
    float square_distance =
        std::abs( possibleCylinderPoint.x - xKey ) * std::abs( possibleCylinderPoint.x - xKey ) +
  ...
  if ( cloud->points.size() > 0 && inRange ) {
    ...
    try {
      ...
      listener_.waitForTransform( "/map", cut_around_keypoint.header.frame_id, time,
                                  ros::Duration( 3.0 ) );
    ...
    markerArray_msg_.markers[0].pose.orientation.x = markerArray_msg_.markers[0].pose.orientation.y =
        markerArray_msg_.markers[0].pose.orientation.z = pp.pose.pose.orientation.x;
...
void BarrelDetection::publish_rectangle_for_recf( std::vector<cv::KeyPoint> keypoints,
    ...
    for ( int j = -width; j <= width; j++ ) {
      if ( ( keypoints.at( i ).pt.x + j ) >= 0 && ( keypoints.at( i ).pt.x + j ) < ipl_img.width ) {
        ...
        if ( ( keypoints.at( i ).pt.y - height ) >= 0 ) {
          cvSet2D( &ipl_img, (int)( keypoints.at( i ).pt.y - height ),
    ...
    for ( int k = -height; k <= height; k++ ) {
      if ( ( keypoints.at( i ).pt.y + k ) >= 0 && ( keypoints.at( i ).pt.y + k ) < ipl_img.height ) {
        ...
        if ( ( keypoints.at( i ).pt.x - width ) >= 0 ) {
          cvSet2D( &ipl_img, (int)( keypoints.at( i ).pt.y + k ),
...
void BarrelDetection::dynamic_recf_cb( hector_barrel_detection_nodelet::BarrelDetectionConfig &config,
                                       uint32_t level )
{
