...
def color_to_gray(img):
    if len(img.shape) == 3:
        ...
        raise ValueError("Invalid input shape (" + str(img.shape) + "). Valid: 2, 3")
...
def draw_matches(img1, kp1, img2, kp2, matches, mask, thickness=1):
    ...
    for i, mat in enumerate(matches):
        ...
        # colour blue
        # thickness = 1
        cv2.circle(out, (int(x1), int(y1)), 4, (255, 0, 0), thickness=thickness)
        cv2.circle(out, (int(x2) + cols1, int(y2)), 4, (255, 0, 0), thickness=thickness)
