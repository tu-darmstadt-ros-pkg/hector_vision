...
void ColorDetection::onInit()
{
  ...
  image_sub =
      it_.subscribeCamera( "/arm_rgbd_cam/rgb/image_raw", 10, &ColorDetection::imageCallback, this );
  ...
  color_marker_publisher_ =
      pnh_.advertise<visualization_msgs::MarkerArray>( "marker_filtered_color", 0 );
  imagePercept_pub_ =
...
void ColorDetection::imageCallback( const sensor_msgs::ImageConstPtr &img,
...
void ColorDetection::findCylinder( const sensor_msgs::PointCloud2::ConstPtr &pc_msg, float xKey,
...
void ColorDetection::publish_rectangle_for_recf( std::vector<cv::KeyPoint> keypoints,
...
void ColorDetection::dynamic_recf_cb( hector_color_detection_nodelet::ColorDetectionConfig &config,
                                      uint32_t level )
{
