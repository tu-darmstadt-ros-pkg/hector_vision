...
struct ImageDetections
{
  ...
  explicit ImageDetections(const size_t detectors)
  {
    ...
    for (size_t i = 0; i < detectors; ++i)
      detectors_detections.emplace_back();
  }
...
public:
                ...
                const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info) override
  {
    ...
    detections_.emplace(image_time, std::pair(
                    ...
                    const rclcpp::MessageInfo& info) override
  {
    ...
    if (auto detector_entry = known_detectors_.find(gid_array); detector_entry != known_detectors_.end())
    {
    ...
    {
      RCLCPP_DEBUG_STREAM(node_->get_logger(), "Could not insert detection from " << detector_name
  ...
  std::pair<cv_bridge::CvImageConstPtr, std::vector<DetectionEntry>> GetAggregatedData() override
  {
    ...
    for (const auto& obsolete_time : times_before_frame)
      detections_.erase(obsolete_time);
  ...
  void UpdatePublishers(const std::vector<rclcpp::TopicEndpointInfo>& publisher_info) override
  {
    ...
    if (!lost_publishers.empty())
    {
      ...
      for (const std::string& name : lost_publishers)
        info_log << name << ", ";
      info_log << std::endl;
    }
    if (!new_publishers.empty())
    {
      ...
      for (const std::string& name : new_publishers)
        info_log << name << ", ";
      info_log << std::endl;
    }
