  ...
  : enabled_(true), has_subscribers_(false)
{
  ...
  image_detected_pub_ = image_transport_->advertiseCamera(
    robot_namespace_ + "/" + aggregation_topic, 10);
...
void DetectionAggregatorNode::createImage()
{
  ...
  RCLCPP_DEBUG(node_->get_logger(), "Creating detection Image for %lu detections", detections.size());
...
{
  {
    ...
    RCLCPP_DEBUG_STREAM(node_->get_logger(), "Aggregating " << perceptions << " Perception" << (perceptions != 1 ? "s" : "")
      << " from " << info.get_rmw_message_info().publisher_gid.data);
  }
...
{
  std::vector<rclcpp::TopicEndpointInfo> publishers = node_->get_publishers_info_by_topic( "/image" );
  RCLCPP_INFO(node_->get_logger(), "Starting subscribers");
  camera_subscriber_ = image_transport_->subscribeCamera("/image", 1, &DetectionAggregatorNode::imageCallback, this);
