...
HeatDetection::HeatDetection( ros::NodeHandle &n, ros::NodeHandle &p_n )
    : image_sub_( p_n, "thermal/image", 1 ), cam_info_sub_( p_n, "thermal/camera_info", 1 ),
      ...
      time_sync_( SyncPolicy( 10 ), image_sub_, cam_info_sub_, image_mapped_sub_ )
{
  ...
  processing_enabled_sub_ =
      p_n.subscribe( "set_processing_enabled", 1, &HeatDetection::setProcessingEnabledCb, this );
                                   ...
                                   const sensor_msgs::ImageConstPtr &img_mapped )
{
  ...
  if ( !mappingDefined_ ) {
    ...
    if ( ( img_thres_.rows != static_cast<int>( img->height ) ) ||
         ( img_thres_.cols != static_cast<int>( img->width ) ) ) {
    ...
    int minThreshold = (int)std::max( std::min( ( ( minTempVictim_ - min_temp_img_ ) *
                                                  ( 256.0 / ( max_temp_img_ - min_temp_img_ ) ) ),
                                                255.0 ),
                                      0.0 );
    int maxThreshold = (int)std::max( std::min( ( ( maxTempVictim_ - min_temp_img_ ) *
                                                  ( 256.0 / ( max_temp_img_ - min_temp_img_ ) ) ),
                                                255.0 ),
                                      0.0 );
    ...
    for ( unsigned int i = 0; i < keypoints.size(); i++ ) {
      ...
      cv::Rect rect_roi( k.pt.x - ( k.size - 1 ) / 2, k.pt.y - ( k.size - 1 ) / 2, k.size - 1,
                         k.size - 1 );
