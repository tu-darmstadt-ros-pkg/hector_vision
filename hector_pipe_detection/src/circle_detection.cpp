...
void DebugInfo::publishFilteredContours( const cv::Mat &image,
                                         std::vector<std::vector<cv::Point>> contours )
{
  cv::Mat out_image = image.clone();
  cv::drawContours( out_image, contours, -1, cv::Scalar( 0, 255, 0 ), 2 );
  filtered_contours_pub_.publish(
      cv_bridge::CvImage( std_msgs::Header(), "rgb8", out_image ).toImageMsg() );
}

void DebugInfo::publishSubEdgeImage( const cv::Mat &sub_edge_image )
{
  sub_edge_image_pub_.publish(
      cv_bridge::CvImage( std_msgs::Header(), "8UC1", sub_edge_image ).toImageMsg() );
}

void DebugInfo::publishSubImageContours( const cv::Mat &sub_image,
                                         std::vector<std::vector<cv::Point>> contours,
                                         const cv::Point &offset )
{
  cv::Mat out_image = sub_image.clone();
  cv::drawContours( out_image, contours, -1, cv::Scalar( 0, 255, 0 ), 2, cv::LINE_8, cv::noArray(),
                    0x7fffffff, offset );
  sub_image_contours_pub_.publish(
      cv_bridge::CvImage( std_msgs::Header(), "rgb8", out_image ).toImageMsg() );
}
...
bool findCircles( const std::vector<std::vector<cv::Point>> &contours, int downsample_passes,
                  ...
                  std::vector<int> &largest_group, int &largest )
{
  ...
  for ( int i = 0; i < contours.size(); ++i ) {
    ...
    if ( area < 500 ||
         cv::abs( square( multiplier * cv::arcLength( contours[i], true ) ) / ( 4 * M_PI * area ) -
                  1 ) > 0.2 ) {
      continue;
    }
  ...
  for ( int i = 1; i < centers.size(); ++i ) {
    ...
    if ( i - start > largest_group_length ||
         ( i - start == largest_group_length && group_area > largest_group_area ) ) {
...
void circlify( std::vector<cv::Point> contour, cv::Point2d &center, double &radius )
{
  ...
  for ( int i = 0; i < contour.size(); ++i ) {
    if ( cv::abs( distances[i] - mean ) > stddev )
      continue;
  ...
  stddev = 0;
  for ( int i = 0; i < filtered_contour.size(); ++i ) {
    stddev += ( distances[filtered_contour_index[i]] - mean ) *
              ( distances[filtered_contour_index[i]] - mean );
  }
  stddev = cv::sqrt( stddev / ( filtered_contour.size() - 1 ) );

  filtered_contour.clear();
  for ( int i = 0; i < contour.size(); ++i ) {
    if ( cv::abs( distances[i] - mean ) > 1.5 * stddev )
      continue;
    filtered_contour.push_back( contour[i] );
  }
  ...
  center = cv::Point2d( moments.m10 / moments.m00, moments.m01 / moments.m00 );
  mean = 0;
  for ( const auto &point : filtered_contour ) { mean += euclideanDistance( center, point ); }

  radius = mean / filtered_contour.size();
}

bool getCircle( const cv::Point &a, const cv::Point &b, const cv::Point &c, double &x, double &y,
                double &r )
{
  if ( b.x == a.x ) {
    if ( b.x == c.x )
...
void findCircle( const cv::Mat &edges, std::vector<std::vector<cv::Point>> contours,
                 unsigned int scaling_factor, double &cx, double &cy, double &cr )
{
...
bool findOuterCircle( const cv::Mat &image, int downsample_passes, cv::Point2d &center,
                      double &radius, std::shared_ptr<DebugInfo> debug_info )
{
  ...
  if ( downsample_passes != 0 ) {
    ...
    std::cout << "SubImage: " << rect.x << ", " << rect.y << ", " << rect.width << ", "
    ...
    cv::findContours( edges, sub_contours, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE,
