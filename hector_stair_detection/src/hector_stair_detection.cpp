...
HectorStairDetection::HectorStairDetection()
{
  ...
  possible_stairs_cloud_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZI>>(
      "/hector_stair_detection/possible_stairs_cloud", 100, true );
  points_on_line_cloud_debug_ = nh.advertise<pcl::PointCloud<pcl::PointXYZI>>(
      "/hector_stair_detection/point_on_line_debug", 100, true );
  surfaceCloud_pub_debug_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/surfaceCloud_debug", 100, true );
  final_stairs_cloud_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/final_stairs_cloud", 100, true );
  border_of_stairs_pub_ = nh.advertise<visualization_msgs::MarkerArray>(
      "/hector_stair_detection/boarder_of_stairs", 100, true );
  stairs_position_and_orientaion_pub_ = nh.advertise<geometry_msgs::PoseStamped>(
      "/hector_stair_detection/stairs_orientation", 100, true );
  stairs_position_and_orientaion_with_direction_pub_ =
      nh.advertise<hector_stair_detection_msgs::PositionAndOrientaion>(
          "/hector_stair_detection/stairs_orientaion_as_vector", 100, true );
  border_and_orientation_stairs_combined_pub_ =
      nh.advertise<hector_stair_detection_msgs::BorderAndOrientationOfStairs>(
          "/hector_stair_detection/border_and_orientation_of_stairs", 100, true );
  cloud_after_plane_detection_debug_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/cloud_after_plane_detection_debug_", 100, true );
  line_marker_pub_ =
      nh.advertise<visualization_msgs::Marker>( "/hector_stair_detection/stairs_normal", 100, true );

  temp_orginal_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/temp_original", 100, true );
  temp_after_pass_trough_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/temp_after_pass_trough", 100, true );
  temp_after_voxel_grid_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(
      "/hector_stair_detection/temp_after_voxel_grid", 100, true );
  temp_after_mls_pub_ = nh.advertise<pcl::PointCloud<pcl::PointNormal>>(
      "/hector_stair_detection/temp_after_mls", 100, true );
  ...
  if ( setup_.compare( std::string( "argo" ) ) == 0 ) {
    ...
    pcl_sub = nh.subscribe( "/worldmodel_main/pointcloud_vis", 10,
                            &HectorStairDetection::PclCallback, this );
  } else {
    ...
    pcl_sub = nh.subscribe( "/hector_aggregate_cloud/aggregated_cloud", 1,
                            &HectorStairDetection::PclCallback, this );
  }
...
void HectorStairDetection::publishResults( pcl::PointCloud<pcl::PointNormal>::Ptr &input_surface_cloud,
                                           ...
                                           std::vector<int> final_cluster_idx, Eigen::Vector3f base,
                                           Eigen::Vector3f point )
{

  geometry_msgs::PoseStamped position_and_orientaion;
  Eigen::Vector3f directionStairs;
  getStairsPositionAndOrientation( base, point, input_surface_cloud->header.frame_id,
                                   directionStairs, position_and_orientaion );
  ...
  getFinalStairsCloud_and_position(
      input_surface_cloud->header.frame_id, directionStairs, input_surface_cloud, planeCloud,
      cluster_indices, final_cluster_idx, final_stairsCloud, stairs_boarder_marker, base );
...
void HectorStairDetection::getStairsPositionAndOrientation(
    Eigen::Vector3f &base, Eigen::Vector3f point, std::string frameID, Eigen::Vector3f &direction,
    geometry_msgs::PoseStamped &position_and_orientaion )
{
  ...
  float staris_pitch = atan2( direction( 1 ) * sin( stairs_yaw ) + direction( 0 ) * cos( stairs_yaw ),
                              direction( 2 ) ) +
                       M_PI_2;
  tf::Quaternion temp;
...
void HectorStairDetection::getFinalStairsCloud_and_position(
    std::string frameID, Eigen::Vector3f directionS,
    pcl::PointCloud<pcl::PointNormal>::Ptr &input_surface_cloud,
    pcl::PointCloud<pcl::PointXYZ>::Ptr &planeCloud, pcl::IndicesClustersPtr cluster_indices,
    std::vector<int> final_cluster_idx, pcl::PointCloud<pcl::PointXYZ>::Ptr &final_stairsCloud,
    visualization_msgs::MarkerArray &stairs_boarder_marker, Eigen::Vector3f base )
{
  ...
  for ( int j = 0; j < ( *cluster_indices )[minZIndex].indices.size(); ++j ) {
    minStepAVG( 0 ) =
        minStepAVG( 0 ) + input_surface_cloud->points[( *cluster_indices )[minZIndex].indices[j]].x;
    minStepAVG( 1 ) =
        minStepAVG( 1 ) + input_surface_cloud->points[( *cluster_indices )[minZIndex].indices[j]].y;
    minStepAVG( 2 ) =
        minStepAVG( 2 ) + input_surface_cloud->points[( *cluster_indices )[minZIndex].indices[j]].z;
  }
  ...
  for ( int j = 0; j < ( *cluster_indices )[maxZIndex].indices.size(); ++j ) {
    maxStepAVG( 0 ) =
        maxStepAVG( 0 ) + input_surface_cloud->points[( *cluster_indices )[maxZIndex].indices[j]].x;
    maxStepAVG( 1 ) =
        maxStepAVG( 1 ) + input_surface_cloud->points[( *cluster_indices )[maxZIndex].indices[j]].y;
    maxStepAVG( 2 ) =
        maxStepAVG( 2 ) + input_surface_cloud->points[( *cluster_indices )[maxZIndex].indices[j]].z;
  }
    ...
    if ( ( directionS( 0 ) > 0 && directionS( 1 ) > 0 ) ||
      ...
      if ( i == 0 ) {
        marker.pose.position.x =
            minStepAVG( 0 ) - 0.5 * sin( atan2( directionS( 1 ), directionS( 0 ) ) );
        marker.pose.position.y =
            minStepAVG( 1 ) + 0.5 * cos( atan2( directionS( 1 ), directionS( 0 ) ) );
      ...
      if ( i == 2 ) {
        marker.pose.position.x =
            maxStepAVG( 0 ) - 0.5 * sin( atan2( directionS( 1 ), directionS( 0 ) ) );
        marker.pose.position.y =
            maxStepAVG( 1 ) + 0.5 * cos( atan2( directionS( 1 ), directionS( 0 ) ) );
...
void HectorStairDetection::projectStairsToFloor( Eigen::Vector3f direction,
                                                 visualization_msgs::MarkerArray &stairs_boarder_marker )
{
  ...
  stairs_boarder_marker.markers.at( minPos1 ).pose.position.x =
      stairs_boarder_marker.markers.at( minPos1 ).pose.position.x +
      ( first_step_z - stairs_boarder_marker.markers.at( minPos1 ).pose.position.z ) /
          direction( 2 ) * direction( 0 );
  stairs_boarder_marker.markers.at( minPos1 ).pose.position.y =
      stairs_boarder_marker.markers.at( minPos1 ).pose.position.y +
      ( first_step_z - stairs_boarder_marker.markers.at( minPos1 ).pose.position.z ) /
          direction( 2 ) * direction( 1 );
  stairs_boarder_marker.markers.at( minPos1 ).pose.position.z = first_step_z;
  stairs_boarder_marker.markers.at( minPos2 ).pose.position.x =
      stairs_boarder_marker.markers.at( minPos2 ).pose.position.x +
      ( first_step_z - stairs_boarder_marker.markers.at( minPos2 ).pose.position.z ) /
          direction( 2 ) * direction( 0 );
  stairs_boarder_marker.markers.at( minPos2 ).pose.position.y =
      stairs_boarder_marker.markers.at( minPos2 ).pose.position.y +
      ( first_step_z - stairs_boarder_marker.markers.at( minPos2 ).pose.position.z ) /
          direction( 2 ) * direction( 1 );
...
int HectorStairDetection::getZComponent( Eigen::Vector2f directionStairs, Eigen::Vector2f minXminY,
                                         Eigen::Vector2f maxXminY, Eigen::Vector2f minXmaxY )
{
...
void HectorStairDetection::getPreprocessedCloud( pcl::PointCloud<pcl::PointXYZ>::Ptr &input_cloud,
...
bool customRegionGrowing( const pcl::PointNormal &point_a, const pcl::PointNormal &point_b,
...
void HectorStairDetection::PclCallback( const sensor_msgs::PointCloud2::ConstPtr &pc_msg )
{
  ...
  try {
    ...
    listener_.waitForTransform( worldFrame_, pc_msg->header.frame_id, time, ros::Duration( 3.0 ) );
    listener_.lookupTransform( worldFrame_, pc_msg->header.frame_id, time, transform_cloud_to_map );
  ...
  for ( int i = 0; i < clusters->size(); ++i ) {
    ...
    if ( fabs( max_x - min_x ) > maxClusterXYDimension_ ||
         fabs( max_y - min_y ) > maxClusterXYDimension_ ) {
  ...
  if ( avg_point_per_cluster.size() >= 2 ) {
    ...
    for ( int i = 0; i < point_in_line_counter.size(); i++ ) { point_in_line_counter.at( i ) = 0; }
      ...
      for ( int i = 0; i < avg_point_per_cluster.size() - 1; i++ ) {
        for ( int j = i + 1; j < avg_point_per_cluster.size(); j++ ) {
          ...
          for ( int p = 0; p < avg_point_per_cluster.size(); p++ ) {
            ...
            float tempA =
                std::sqrt( direction.cross( point - base ).dot( direction.cross( point - base ) ) );
    ...
    while ( 1 ) {
      ...
      if ( pointsAtLineCounter >= minCountPointsAtLine ) {
        ...
        if ( maxDistBetweenPoints( debug_line_cloud ) <= maxDistBetweenStairsPoints_ &&
             minHightDistBetweenPoints( debug_line_cloud ) >= minHightDistBetweenAllStairsPoints_ ) {
          ...
          publishResults( input_surface_cloud, planeCloud, clusters, final_cluster_idx, base,
                          direction + base );
        } else {
          ROS_INFO( "No stairs, distance between points to large, or heightDistance between point "
                    "too small" );
        }
...
void HectorStairDetection::stairsSreachPlaneDetection(
    pcl::PointCloud<pcl::PointNormal>::Ptr &input_surface_cloud,
    pcl::PointCloud<pcl::PointXYZI>::Ptr points_on_line, Eigen::Vector3f base, Eigen::Vector3f dir,
    pcl::PointCloud<pcl::PointXYZ>::Ptr &planeCloud )
{
  ...
  pass.setInputCloud( processCloud_v1 );
  pass.setFilterFieldName( "y" );
  pass.setFilterLimits( base( 1 ) - fabs( sin( angleXToStairs ) * maxDistBetweenStairsPoints_ ) -
                            maxClusterXYDimension_ / 2,
                        base( 1 ) + fabs( sin( angleXToStairs ) * maxDistBetweenStairsPoints_ ) +
                            maxClusterXYDimension_ / 2 );
  pass.filter( *processCloud_v2 );

  pass.setInputCloud( processCloud_v2 );
  pass.setFilterFieldName( "x" );
  pass.setFilterLimits( base( 0 ) - fabs( cos( angleXToStairs ) * maxDistBetweenStairsPoints_ ) -
                            maxClusterXYDimension_ / 2,
                        base( 0 ) + fabs( cos( angleXToStairs ) * maxDistBetweenStairsPoints_ ) +
                            maxClusterXYDimension_ / 2 );
...
float HectorStairDetection::maxDistBetweenPoints( pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud )
{
  ...
  for ( int i = 0; i < input_cloud->size(); i++ ) {
    for ( int j = i; j < input_cloud->size(); j++ ) {
      temp_dist = std::sqrt( std::pow( input_cloud->at( i ).x - input_cloud->at( j ).x, 2 ) +
                             std::pow( input_cloud->at( i ).y - input_cloud->at( j ).y, 2 ) +
                             std::pow( input_cloud->at( i ).z - input_cloud->at( j ).z, 2 ) );
...
void HectorStairDetection::dynamic_recf_cb( hector_stair_detection::HectorStairDetectionConfig &config,
                                            uint32_t level )
{
