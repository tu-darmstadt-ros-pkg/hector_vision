...
void getJacobians( const cv::Mat &response_x, const cv::Mat &response_y, cv::Mat &jx, cv::Mat &jy,
                   cv::Mat &jxy )
{
  cv::Mat channels_x[3];
...
void internalCalculateColorEdges( const cv::Mat &image, cv::Mat &edges, cv::Mat &orientation,
                                  bool include_orientation, int filter_type )
{
  ...
  for ( int row = 0; row < edges.rows; ++row ) {
    for ( int col = 0; col < edges.cols; ++col ) {
      orientation.at<float>( row, col ) = cv::fastAtan2(
          -jxy.at<float>( row, col ), e1.at<float>( row, col ) - jy.at<float>( row, col ) );
    }
  }
}
                          ...
                          bool include_orientation )
{
  if ( image.channels() != 3 ) {
    char buffer[256];
    sprintf( buffer,
             "Unsupported number of channels. Only 3 channel color images are supported! "
             "Number of channels was: %d",
             image.channels() );
  ...
  if ( image.depth() == CV_8U ) {
    ...
    char buffer[256];
    sprintf( buffer,
             "Depth not supported! Supported depths are CV_8U and CV_32F. "
             "Depth was: %d",
             image.depth() );
    throw std::runtime_error( buffer );
  }
