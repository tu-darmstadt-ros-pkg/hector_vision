...
template<typename T>
void internalCalculateThresholds( const cv::Mat &mat, double &upper, double &lower,
                                  double stop_val ) // TODO special cases can be faster
{
  double old_upper = 0;
  double old_lower = 0;
  double max = 0;
...
void calculateThresholds( const cv::Mat &image, double &upper, double &lower, double stop_val )
{
  if ( image.channels() != 1 ) {
    CV_Error( CV_BadNumChannels,
              "Calculating an upper and lower threshold only supports single channel images!" );
  }
  switch ( image.type() ) {
  case CV_8S:
...
inline void flowThreshold( const cv::Mat &edges, cv::Mat &binary, double lower, int row, int col )
{
  for ( int i = -1; i <= 1; ++i ) {
    for ( int j = -1; j <= 1; ++j ) {
      ...
      if ( c < 0 || c == edges.cols )
        continue;
      if ( binary.at<unsigned char>( r, c ) == 255 || edges.at<T>( r, c ) < lower )
        continue;
...
cv::Mat threshold( const cv::Mat &image, double upper, double lower )
{
  if ( image.channels() != 1 ) {
    CV_Error( CV_BadNumChannels,
              "Threshold with upper and lower threshold only supports single channel images!" );
  }
  switch ( image.type() ) {
  case CV_8S:
  case CV_8SC2:
