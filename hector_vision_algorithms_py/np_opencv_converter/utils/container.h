...
template<typename T>
struct expose_template_type<std::vector<T>> : public expose_template_type_base<std::vector<T>> {
  ...
  expose_template_type()
  {
    ...
    if ( !base_type::wrapped() ) {
      ...
      py::converter::registry::push_back( this_type::convertible, this_type::construct,
  ...
  {
    py::list l;
    for ( typename wrapped_type::const_iterator iter = container.begin(); iter != container.end();
          iter++ ) {
