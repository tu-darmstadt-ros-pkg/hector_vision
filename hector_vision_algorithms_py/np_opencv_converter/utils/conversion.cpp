...
public:
    ...
    UMatData* allocate(int dims0, const int* sizes, int type, void* data, size_t* step, cv::AccessFlag flags,
                       cv::UMatUsageFlags uMatUsageFlags) const
    {
        if( data != 0 )
        {
            CV_Error(Error::StsAssert, "The data should normally be NULL!");
            // probably this is safe to do in such extreme case
            return stdAllocator->allocate(dims0, sizes, type, data, step, flags, uMatUsageFlags);
        }
        PyEnsureGIL gil;
    ...
    bool allocate(UMatData* u, cv::AccessFlag accessFlags, cv::UMatUsageFlags uMatUsageFlags) const
    {
        return stdAllocator->allocate(u, accessFlags, uMatUsageFlags);
    }
