...
BOOST_PYTHON_MODULE( libhector_vision_algorithms_py )
{
  ...
  def<tuple ( * )( const cv::Mat & )>( "color_edges", &calculateColorEdges,
                                       "edges, orientation = color_edges( image )" );
  def<tuple ( * )( const cv::Mat &, bool )>(
      "color_edges", &calculateColorEdges,
      "edges, orientation = color_edges( image, include_orientation )" );

  // Thresholding
  def<tuple ( * )( const cv::Mat & )>( "calculate_thresholds", &calculateThresholds,
                                       "upper, lower = calculate_thresholds( image )" );
  def<tuple ( * )( const cv::Mat &, double )>(
      "calculate_thresholds", &calculateThresholds,
  ...
  def<cv::Mat ( * )( const cv::Mat &, double, double )>(
      "threshold", &hector_vision_algorithms::threshold,
