    ...
    : current_image_( 0 ), node_( node ), image_transport_( node ), current_image_frame_( 0 )
{
  ...
  node_->declare_parameter( "image_dir",
                            ament_index_cpp::get_package_share_directory( "hector_vision_test" ) +
                                "/images" );
  ...
  uint file_count = 0;
  for ( const auto &file : fs::directory_iterator( image_dir_ ) ) ++file_count;
  ...
  for ( const auto &file : fs::directory_iterator( image_dir_ ) ) {
    try {
      ...
      RCLCPP_ERROR_STREAM( node_->get_logger(),
                           "Could not load image file " << file.path() << ": " << e.what() );
      throw;
    }
  }

  camera_publisher_ = image_transport_.advertiseCamera( node_->get_parameter("image_topic").as_string(), 10 );

  publish_timer_ = node_->create_wall_timer(
      std::chrono::milliseconds(
          static_cast<int>( 1000 / ( image_frequency * static_cast<double>( image_frames_ ) ) ) ),
...
void CameraDummyNode::publishImage()
{
  ...
  cv_bridge::CvImage( header, sensor_msgs::image_encodings::BGR8, images_[current_image_] )
      .toImageMsg( image );
  image.header = header;
